...
[best,FSE]=synthesize(filename,jobj,payload,ORIGINAL,QF);  %获取最好PSNR与其对应的FSE
FSE_each = FSE;
PSNR_each = best(2,:);
SSIM_each = best(3,:);
...
%测试数据放入ave_g512
save(['ave_g512\payload_', num2str(QF), '.mat'],'payload');
save(['ave_g512\psnr_', num2str(QF), '.mat'],'psnr_ave');
save(['ave_g512\filesize_', num2str(QF), '.mat'],'filesize_ave');
save(['ave_g512\ssim_', num2str(QF), '.mat'],'ssim_ave');

%每个的实验结果也放到ave_ucid中
save(['ave_g512\result_each', num2str(QF), '.mat'],'result');
end
